...
vim.keymap.set('n', '+-', "<cmd>:lua require('quicknote').DeleteNoteAtCurrentLine()<cr>", { desc = 'Delete note at current line' })
...
vim.keymap.set('n', '+l', "<cmd>:lua require('quicknote').ListNotesForCurrentBuffer()<cr>", { desc = 'Show list of notes' })
vim.keymap.set('n', '+a', "<cmd>:lua require('quicknote').NewNoteAtCurrentLine()<cr>", { desc = 'Create new notes on current line' })
vim.keymap.set('n', '+o', "<cmd>:lua require('quicknote').OpenNoteAtCurrentLine()<cr>", { desc = 'Open notes on current line' })
...

  {
    ...
    opts = {
      ...
      formatters_by_ft = {
        ...
        python = { 'isort', 'black' },
        javascript = {
          -- Check for .prettierrc in the project root
          command = function()
            local project_root = vim.fn.getcwd()
            local prettierrc_exists = vim.fn.filereadable(project_root .. '/.prettierrc') == 1

            if prettierrc_exists then
              return 'prettier'
            else
              return 'prettierd'
            end
          end,
          stop_after_first = true, -- Use the first successful formatter
        },
        typescript = {
          command = function()
            local project_root = vim.fn.getcwd()
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      require('mini.surround').setup {
        mappings = {
          add = 'ys', -- 'ys' for adding surrounding (Vim-style)
          delete = 'ds', -- 'ds' for deleting surrounding (Vim-style)
          replace = 'cs', -- 'cs' for changing surrounding (Vim-style)
          find = '', -- Optionally leave empty for now
          find_left = '', -- Optionally leave empty for now
          highlight = '', -- Optionally leave empty for now
          update_n_lines = '', -- Optionally leave empty for now
        },
      }
